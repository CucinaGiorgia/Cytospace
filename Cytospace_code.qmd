...
```{r Data upload}
nuc_og_ctrl <- read.csv(here::here("nuc_og_ctrl.csv")) %>% 
  janitor::clean_names() %>% 
  dplyr::mutate(label="og_ctrl") %>% 
  dplyr::filter(!is.na(area)) %>% 
...
nuc_og_mel <- read.csv(here::here("nuc_og_mel.csv")) %>% 
  janitor::clean_names() %>% 
  dplyr::mutate(label="og_mel") %>% 
  dplyr::filter(!is.na(area)) %>% 
    dplyr::filter(area>= 1000) %>% 
  dplyr::group_by(label) %>% 
    dplyr::mutate(
...
nuc_rpm <- read.csv(here::here("nuc_rpm.csv")) %>% 
  janitor::clean_names() %>% 
  dplyr::mutate(label="rpm_ctrl") %>% 
  dplyr::filter(!is.na(area)) %>% 
    dplyr::filter(area>= 1000) %>% 
  dplyr::group_by(label) %>% 
    dplyr::mutate(
...
nuc_docking <- read.csv(here::here("nuc_docking.csv")) %>% 
  janitor::clean_names() %>% 
  dplyr::mutate(label="rpm_docking") %>% 
  dplyr::filter(!is.na(area)) %>% 
  dplyr::filter(area>= 1000) %>% 
  dplyr::group_by(label) %>% 
    dplyr::mutate(
...
space_nuc_ctrl <- read.csv(here::here("space_nuc_ctrl.csv")) %>% 
  janitor::clean_names() %>% 
  dplyr::mutate(label="space_ctrl") %>% 
  dplyr::filter(!is.na(area)) %>% 
  dplyr::filter(area>= 1000) %>% 
  dplyr::group_by(label) %>% 
    dplyr::mutate(
...
space_nuc_docking <- read.csv(here::here("space_nuc_docking.csv")) %>% 
  janitor::clean_names() %>% 
  dplyr::mutate(label="space_docking") %>% 
  dplyr::filter(!is.na(area)) %>% 
  dplyr::filter(area>= 1000) %>% 
  dplyr::group_by(label) %>% 
    dplyr::mutate(
...
space_nuc_mel <- read.csv(here::here("space_nuc_mel.csv")) %>% 
  janitor::clean_names() %>% 
  dplyr::mutate(label="space_mel") %>% 
  dplyr::filter(!is.na(area)) %>% 
  dplyr::filter(area>= 1000) %>% 
  dplyr::group_by(label) %>% 
    dplyr::mutate(
...
nuclei <- rbind(nuc_og_ctrl, nuc_og_mel, nuc_rpm, space_nuc_ctrl, space_nuc_mel) 
 
# Roundness   
ggplot2::ggplot(nuclei, aes(x = factor(
    label,
    levels = c(
      "og_ctrl",
      "rpm_ctrl",
  ...
  labs(title = "Roundness Nuclei", x = "Label", y = "Mean") +
  geom_text(aes(
    label = round(mean_round, 3),
    vjust = 15,
    size = 3
  ...
  geom_hline(yintercept = 0)+
  theme(plot.title = element_text(hjust = 0.5), legend.position = "none")+
  scale_fill_manual(values=c("#6a994e",
    "#a7c957",
    "#fdc43f",
    "#ffdb57",
    "#02cecb",
...
ggplot2::ggplot(nuclei, aes(x = factor(
  ...
  labs(title = "Area Nuclei", x = "Label", y = "Area") +
  geom_text(aes(
    label = round(mean_area, 0),
    vjust = 12,
    size = 3
  )) +  
  geom_hline(yintercept = 0)+
...
```{r p_values for nuclei}
p_values <- nuclei %>%
  dplyr::mutate(p_value_solidity = case_when(
    label %in% c("og_mel", "og_ctrl") ~ t.test(solidity ~ label, data = nuclei %>% dplyr::filter(label %in% c("og_mel", "og_ctrl")))$p.value,
    label %in% c("rpm_ctrl", "og_ctrl") ~ t.test(solidity ~ label, data = nuclei %>% dplyr::filter(label %in% c("rpm_ctrl", "og_ctrl")))$p.value,
    label %in% c("space_ctrl", "og_ctrl") ~ t.test(solidity ~ label, data = nuclei %>% dplyr::filter(label %in% c("space_ctrl", "og_ctrl")))$p.value,
    label %in% c("space_mel", "og_ctrl") ~ t.test(solidity ~ label, data = nuclei %>% dplyr::filter(label %in% c("space_mel", "og_ctrl")))$p.value,
    TRUE ~ NA_real_  # Assign NA for labels not matching any of the conditions
  )) %>%
    dplyr::mutate(p_value_area= case_when(
    label %in% c("og_mel", "og_ctrl") ~ t.test(area ~ label, data = nuclei %>% dplyr::filter(label %in% c("og_mel", "og_ctrl")))$p.value,
    label %in% c("rpm_ctrl", "og_ctrl") ~ t.test(area ~ label, data = nuclei %>% dplyr::filter(label %in% c("rpm_ctrl", "og_ctrl")))$p.value,
    label %in% c("space_ctrl", "og_ctrl") ~ t.test(area ~ label, data = nuclei %>% dplyr::filter(label %in% c("space_ctrl", "og_ctrl")))$p.value,
    label %in% c("space_mel", "og_ctrl") ~ t.test(area ~ label, data = nuclei %>% dplyr::filter(label %in% c("space_mel", "og_ctrl")))$p.value,
    TRUE ~ NA_real_  # Assign NA for labels not matching any of the conditions
  )) %>% 
      dplyr::mutate(p_value_round = case_when(
    label %in% c("og_mel", "og_ctrl") ~ t.test(round ~ label, data = nuclei %>% dplyr::filter(label %in% c("og_mel", "og_ctrl")))$p.value,
    label %in% c("rpm_ctrl", "og_ctrl") ~ t.test(round ~ label, data = nuclei %>% dplyr::filter(label %in% c("rpm_ctrl", "og_ctrl")))$p.value,
    label %in% c("space_ctrl", "og_ctrl") ~ t.test(round ~ label, data = nuclei %>% dplyr::filter(label %in% c("space_ctrl", "og_ctrl")))$p.value,
    label %in% c("space_mel", "og_ctrl") ~ t.test(round ~ label, data = nuclei %>% dplyr::filter(label %in% c("space_mel", "og_ctrl")))$p.value,
    TRUE ~ NA_real_  # Assign NA for labels not matching any of the conditions
  )) %>% 
  distinct(label, .keep_all = TRUE) %>% 
  dplyr::mutate(p_value_solidity= round(p_value_solidity, 5),
                p_value_round= round(p_value_round, 5),
                p_value_area= round(p_value_area, 5)) %>% View()
